...
class LLMTrainer(Trainer):
    ...
    def generate_candidates(self,reranked_data_path,text_dic):
        self.model.eval()
        test_topk_items = []
        with torch.no_grad():
            ...
            for batch_idx, batch in enumerate(tqdm_dataloader):
                ...
                # 处理logits获取scores
                scores = self.verbalizer.process_logits(logits)
                # 获取top-k的索引
                k = 10  # 可以自定义k值
                topk_scores, topk_indices = torch.topk(scores, k, dim=1)
                batch_topk_items = []
                for user_idx in range(len(input_ids)):
                    user_topk_items = []
                    for rank, (item_idx, score) in enumerate(zip(topk_indices[user_idx], topk_scores[user_idx])):
                        item_id = item_idx.item()
                        item_info = text_dic[item_id] if item_id in text_dic else "Unknown"
                        user_topk_items.append({
                            'item_id': item_id,
                            'score': score.item(),
                            'rank': rank + 1,
                            'item_info': item_info
                        })
                    batch_topk_items.append(user_topk_items)
                test_topk_items.extend(batch_topk_items)



        with open(reranked_data_path, 'wb') as f:
            pickle.dump({' test_topk_items':  test_topk_items,
                         }, f)
