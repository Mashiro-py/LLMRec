...
def generate_rating_matrix_test(user_seq, num_users, num_items):
    ...
    col = []
    data = []
    for user_id, item_list in tqdm(enumerate(user_seq)):
...
def get_user_seqs(data_file):
    """
        功能: 使用 pandas 读取数据, 相比于读取 txt 后 append 速度飞速提升, 该函数基本几秒可以完成
        参数:
            data_file: 输入的 csv 文件
    """
    ...
    # 获得数据
    user_seq = list(sample_data['seq'])
    user_type_seq = list(sample_data['type_seq'])

    # 数据类型处理
    user_seq = list(map(lambda x: eval(x), user_seq))
    user_type_seq = list(map(lambda x: eval(x), user_type_seq))

    # 处理 item
    item_set = set()
    type_item_set = set()
    for sublist in user_seq:
        for element in sublist:
            item_set.add(element)
    for sublist in user_type_seq:
        for element in sublist:
            type_item_set.add(element)


    max_item = max(item_set)
    max_type_item = max(type_item_set)


    num_users = len(sample_data)
    num_items = max_item + 2
    num_type_items = max_type_item + 2

    valid_rating_matrix = generate_rating_matrix_valid(user_seq, num_users, num_items)
    test_rating_matrix = generate_rating_matrix_test(user_seq, num_users, num_items)

    valid_rating_matrix_extend = generate_rating_matrix_valid(user_type_seq, num_users, num_type_items)
    test_rating_matrix_extend = generate_rating_matrix_test(user_type_seq, num_users, num_type_items)

    return user_seq, max_item, valid_rating_matrix, test_rating_matrix, num_users, user_type_seq, max_type_item
...
def cal_mrr(actual, predicted):
    ...
    for i in range(num_users):
        ...
        if np.sum(r) > 0:
            ...
            one_user_mrr = np.reciprocal(np.where(r==1)[0]+1, dtype=np.cfloat)[0]
...
class CrossModule(nn.Module):
    def __init__(
        ...
        self.corre_dim = corre_dim
        self.corre_out_dim = corre_out_dim
        self.c_specific = nn.Sequential(
            nn.Linear(self.corre_dim, 256),
            nn.BatchNorm1d(256),
            nn.ReLU(),
            nn.Dropout(),
            nn.Linear(256, self.corre_out_dim),
            nn.BatchNorm1d(self.corre_out_dim),
            nn.ReLU()
        )
...
class CrossModule(nn.Module):
    def __init__(
        ...
        self.c_specific_3 = nn.Sequential(
            nn.Linear(corre_in_dim, hidden_layer),
            nn.LayerNorm(hidden_layer),
            nn.ReLU(),
            nn.Dropout(p=0.1),
            nn.Linear(hidden_layer, corre_out_dim),
            nn.LayerNorm(corre_out_dim),
            nn.ReLU()
        )
